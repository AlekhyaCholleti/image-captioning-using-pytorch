...
def build_vocab(json,threshold):
	...
	for i, id in enumerate(ids):
		...
		if (i+1) % 1000 == 0:
			print("[{}/{}] tokenised the captions." .format(i+1,len(ids)))

	#if the word frequency is less than threshold, then the word is discarded
	words = [word for word,cnt in counter.items() if cnt >= threshold]

	#now add the words to vocabulary object and few other special tokens too
	vocab = Vocabulary()
	vocab.add_word("<pad>")
...
def main(args):
	...
	# storing vocab object in file having vocab_path
